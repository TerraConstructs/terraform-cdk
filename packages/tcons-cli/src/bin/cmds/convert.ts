...
class Command extends BaseCommand {
  ...
  public readonly builder = (args: yargs.Argv) =>
    args
      .example(
        "cat main.tf | tcons convert  --provider integrations/github",
        "Takes the HCL content of main.tf and converts it to CDK for Terraform content and prints it",
      )
      .example(
        "cat main.tf | tcons convert --provider hashicorp/aws > imported.ts",
        "Takes the HCL content of main.tf and converts it to CDK for Terraform content in imported.ts",
      )
      .example(
        "cat main.tf | tcons convert --provider 'hashicorp/aws@ ~>3.62.0' 'integrations/github@ ~>4.16.0' --language python > imported.py",
  ...
  public async handleCommand(argv: any) {
    ...
    // deferred require to keep tcons-cli main entrypoint small (e.g. for fast shell completions)
