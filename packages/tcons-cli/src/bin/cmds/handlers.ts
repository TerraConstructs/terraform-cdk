...
import { convert as hcl2cdkConvert } from "@tcons/hcl2cdk";
import {
  GetOptions,
  TerraformModuleConstraint,
  TerraformProviderConstraint,
} from "@tcons/provider-generator";
...
import { readSchema } from "@tcons/provider-schema";
...
export async function debug(argv: any) {
  // Ideally the provider info gathering happens in `collectDebugInformation()` but the inclusion of `CdktfConfig.read()` in @tcons/commons causes the build to fail with "error TS5055"
  const config = CdktfConfig.read();
