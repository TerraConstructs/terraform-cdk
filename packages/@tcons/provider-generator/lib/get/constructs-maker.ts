...
import { exec, mkdtemp } from "@tcons/commons";
import {
...
import { DISPLAY_VERSION, Language } from "@tcons/commons";
...
export class ConstructsMaker {
  ...
  private async generateJsiiLanguage(target: ConstructsMakerTarget) {
    ...
    const deps = ["@types/node", "constructs", "tcons"];
...
So tcons-cli sets it to NODE_OPTIONS="--max-old-space-size=16384" by default. As your environment already contains
