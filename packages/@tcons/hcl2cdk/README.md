...
Uses `@tcons/hcl2cdk` to transform HCL configuration to CDK constructs.

## Usage

```sh
yarn add @tcons/hcl2cdk
```

### Convert HCL strings to Constructs

```ts
import { convert } from "@tcons/hcl2cdk";
...
```ts
import * as hcl2cdk from "@tcons/hcl2cdk";
import {
  readSchema,
