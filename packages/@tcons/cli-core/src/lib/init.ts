...
import { Errors } from "@tcons/commons";

import { sscaff } from "sscaff";
import { FUTURE_FLAGS } from "tcons/lib/features";
...
export async function determineDeps(
  ...
  // TS: tcons-0.10.1-dev.2160938258
  // Py: tcons-0.10.1.dev1658821493.whl
  ...
  if (dist) {
    const ret = {
      npm_cdktf: path.resolve(dist, "js", `tcons@${version}.jsii.tgz`),
      npm_cdktf_cli: path.resolve(dist, "js", `tcons-cli-${version}.tgz`),
      pypi_cdktf: path.resolve(
        dist,
        "python",
        `tcons-${pythonVersion}-py3-none-any.whl`,
      ),
      mvn_cdktf: path.resolve(
        dist,
        "java",
        `dev/terraconstructs/tcons/${version}/tcons-${version}.jar`,
      ),
      nuget_cdktf: path.resolve(
        dist,
        "dotnet",
        `TerraConstructs.Cdktf.${version}.nupkg`,
      ),
  ...
  return {
    ...
    constructs_version: constructsVersion,
    npm_cdktf: `tcons@${ver}`,
    npm_cdktf_cli: `tcons-cli@${ver}`,
    pypi_cdktf: `tcons~=${pythonVersion}`,
    mvn_cdktf: version,
    nuget_cdktf: version,
    go_cdktf: `v${version}`,
  };
